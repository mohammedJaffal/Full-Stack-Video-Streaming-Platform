...
use crate::{
    ...
    error::AppError,
    models::{
        AdminContentInput, AdminContentItem, Job, JobInput, ProviderHealth, SystemLog,
    },
    state::AppState,
};
...
fn validate_hls_content(input: &AdminContentInput) -> Result<(), AppError> {
    if input.playback_type != "hls" {
        return Err(AppError(
...
pub async fn list_content(
    _: AuthClaims,
    State(state): State<AppState>,
) -> Result<Json<Vec<AdminContentItem>>, AppError> {
    Ok(Json(
        sqlx::query_as::<_, AdminContentItem>(&format!(
            "{ADMIN_CONTENT_SELECT} ORDER BY c.created_at DESC"
        ))
        .fetch_all(&state.db)
        .await?,
    ))
...
pub async fn create_content(
    _: AuthClaims,
    State(state): State<AppState>,
    Json(input): Json<AdminContentInput>,
) -> Result<(StatusCode, Json<Value>), AppError> {
    validate_hls_content(&input)?;
    let result = sqlx::query("INSERT INTO content (title,slug,description,release_year,duration_seconds,poster_url,backdrop_url,category_id,playback_source,playback_type,is_active) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
        .bind(input.title)
        .bind(input.slug)
    ...
    Ok((
        StatusCode::CREATED,
        Json(json!({"id": result.last_insert_id()})),
    ))
}
...
pub async fn update_content(
    ...
    validate_hls_content(&input)?;
    let result = sqlx::query("UPDATE content SET title=?,slug=?,description=?,release_year=?,duration_seconds=?,poster_url=?,backdrop_url=?,category_id=?,playback_source=?,playback_type='hls',is_active=? WHERE id=?")
        .bind(input.title)
        .bind(input.slug)
        .bind(input.description)
        .bind(input.release_year)
    ...
    if result.rows_affected() == 0 {
        return Err(AppError(StatusCode::NOT_FOUND, "content not found".into()));
    }
...
pub async fn create_job(
    ...
    if !matches!(
        input.job_type.as_str(),
        "metadata_enrichment" | "subtitle_import" | "duplicate_detection"
    ) {
        return Err(AppError(
            StatusCode::BAD_REQUEST,
            "unsupported job type".into(),
        ));
    }
    ...
    log(
        ...
        "info",
        "jobs",
        &format!("Queued {} job {}", input.job_type, id),
    )
    .await?;
    Ok((
        StatusCode::CREATED,
...
pub async fn retry_job(
    ...
    let result = sqlx::query("UPDATE ingestion_jobs SET status='queued',progress=0,error_message=NULL,log_message='Queued for retry',started_at=NULL,completed_at=NULL WHERE id=? AND status='failed'")
        .bind(&id)
        .execute(&state.db)
        .await?;
    if result.rows_affected() == 0 {
...
pub async fn job_logs(
    ...
    let row: Option<(String, Option<String>, Option<String>)> = sqlx::query_as(
        "SELECT status,log_message,error_message FROM ingestion_jobs WHERE id=?",
    )
    .bind(id)
    .fetch_optional(&state.db)
    .await?;
    let (status, log_message, error_message) =
        row.ok_or_else(|| AppError(StatusCode::NOT_FOUND, "job not found".into()))?;
    Ok(Json(
        json!({"status":status,"log":log_message,"error":error_message}),
    ))
}
...
async fn log(
    ...
    sqlx::query("INSERT INTO system_logs (level,source,message) VALUES (?,?,?)")
        .bind(level)
        .bind(source)
        .bind(message)
        .execute(&state.db)
...
mod tests {
    ...
    fn input(playback_type: &str, source: &str) -> AdminContentInput {
        AdminContentInput {
            title: "Demo".into(),
            slug: "demo".into(),
            ...
            poster_url: "/poster.svg".into(),
            backdrop_url: "/backdrop.svg".into(),
            category_id: 1,
            playback_source: source.into(),
            playback_type: playback_type.into(),
            is_active: true,
        }
    }
    ...
    fn rejects_non_hls_content() {
        let error = validate_hls_content(&input("mp4", "https://example.com/video.mp4"))
            .unwrap_err();
