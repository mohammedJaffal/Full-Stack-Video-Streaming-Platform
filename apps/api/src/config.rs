...
impl Config {
    pub fn from_env() -> Self {
        Self {
            ...
            playback_ttl_seconds: env::var("PLAYBACK_TOKEN_TTL_SECONDS").ok().and_then(|v| v.parse().ok()).unwrap_or(1800),
            media_relay_base_url: env::var("MEDIA_RELAY_BASE_URL").unwrap_or_else(|_| "http://localhost:8787".into()),
            allowed_origin: env::var("ALLOWED_ORIGIN").unwrap_or_else(|_| "http://localhost:4321".into()),
